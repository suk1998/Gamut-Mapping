...
sRGB_R1 = RotMat*midPnt_RG';
sRGB_R1=sRGB_R1';

Pnt = [sRGB_R1(1) sRGB_R1(2)];
xc1 = [Ax(1) OrignX Pnt(1)];
yc1 = [Ay(1) OrignY Pnt(2)];

sRGB_R1 = RotMat*midPnt_BR';
sRGB_R1=sRGB_R1';

Pnt = [sRGB_R1(1) sRGB_R1(2)];
xc2 = [Ax(1) OrignX Pnt(1)];
yc2 = [Ay(1) OrignY Pnt(2)];

pc1 = polyfit(xc1,yc1,2);
pc2 = polyfit(xc2,yc2,2);

if abs(pc1(1)) > abs(pc2(1))
    R_curvatureMax = abs(pc1(1));
else
    R_curvatureMax = abs(pc2(1));
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%Green curvature distribution%%%%%%%%%%%%%%%
...
if abs(pc1(1)) > abs(pc2(1))
    G_curvatureMax = abs(pc1(1));
else
    G_curvatureMax = abs(pc2(1));
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%blue curvature distribution%%%%%%%%%%%%%%%
...
if abs(pc1(1)) > abs(pc2(1))
    B_curvatureMax = abs(pc1(1));
else
    B_curvatureMax = abs(pc2(1));
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
xn = [0.0: 0.1:max([R_curvatureMax;G_curvatureMax;B_curvatureMax])];
yn = normpdf(xn,MeanCurvature,sigCurvature);
CurvatureRatio = MaxCurvature/max(yn);
yn = normpdf(xn,MeanCurvature,sigCurvature)*CurvatureRatio;
figure(2),
...
R_disMean = max([R_Maxdis G_Maxdis B_Maxdis])*MeanPercent;
G_disMean = max([R_Maxdis G_Maxdis B_Maxdis])*MeanPercent;
B_disMean = max([R_Maxdis G_Maxdis B_Maxdis])*MeanPercent;
...
%%%%%%%%%%% Red distanc distribution %%%%%%%%%%
xn = [0:0.001:R_Maxdis];
yn = normpdf(xn,R_disMean,sig);
Rdegree_ratio = MaxStep/max(yn);
yn = normpdf(xn,R_disMean,sig)*Rdegree_ratio;
figure(3), plot(xn,yn,'r-'),grid on,hold on,
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%% Green distanc distribution %%%%%%%%%
xn = [0:0.001:G_Maxdis];
yn = normpdf(xn,G_disMean,sig);
Gdegree_ratio = MaxStep/max(yn);
yn = normpdf(xn,G_disMean,sig)*Gdegree_ratio;
plot(xn,yn,'g-'),grid on,hold on,
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%% Blue distanc distribution %%%%%%%%%%
xn = [0:0.001:B_Maxdis];
yn = normpdf(xn,B_disMean,sig);
Bdegree_ratio = MaxStep/max(yn);
yn = normpdf(xn,B_disMean,sig)*Bdegree_ratio;
plot(xn,yn,'b-'),grid on,hold off,
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:imgSize,
    ...
    if inR(i)==1,
        ...
        if charmode ==1, % curve expansion 데이터로 보고 싶을 때는 0 아니면 먼셀데이터로 보고 싶으면 1
            ...
            CurAtten = normpdf(abs(p(1)),MeanCurvature,sigCurvature)*CurvatureRatio;
            r_sq = normpdf(r_sq,R_disMean,sig)*Rdegree_ratio*CurAtten;
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        plot(tarPnt(1),tarPnt(2),'bo','markersize',4,'LineWidth',2),
        hold on,
    ...
    if inG(i)==1,
        ...
        if charmode ==1, % curve expansion 데이터로 보고 싶을 때는 0 아니면 먼셀데이터로 보고 싶으면 1
            ...
            CurAtten = normpdf(abs(p(1)),MeanCurvature,sigCurvature)*CurvatureRatio;
            r_sq = normpdf(r_sq,G_disMean,sig)*Gdegree_ratio*CurAtten;
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        plot(tarPnt(1),tarPnt(2),'ro','markersize',4,'LineWidth',2),
        hold on,
    ...
    if inB(i)==1,
        ...
        if charmode ==1, % curve expansion 데이터로 보고 싶을 때는 0 아니면 먼셀데이터로 보고 싶으면 1
            ...
            CurAtten = normpdf(abs(p(1)),MeanCurvature,sigCurvature)*CurvatureRatio;
            r_sq = normpdf(r_sq,B_disMean,sig)*Bdegree_ratio*CurAtten;
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        plot(tarPnt(1),tarPnt(2),'go','markersize',4,'LineWidth',2),
        hold on,
