% Curve Expansion without considering achromatic region (old one)
% When char mode is 1, read xls data and then make curve extension with the
% data. When char mode is 0, it is based on the input image. Thus, it makes
% curve extension with the image itself.
...
if charmode ==1,
    filename = ['Munsell_sRGB_C_D65_photoshop' ];
    formatfile = ['.xlsx'];
    ...
    filename = ['low_resolution_02\\florence-1936780_640' ];
    formatfile = ['.jpg'];
    readfilename = [filename formatfile];
...
for i=1:imgSize,
        ...
        if inR(i)==1,      
            ...
            x2 = linspace(min(x(3),tarPnt(1)),max(x(3),tarPnt(1)),50);
...
% 
% 
% for i=1:1:max(size(sRGB_G)),
% %for i=1:1,
%     
%     Pnt = [sRGB_G1(i,1) sRGB_G1(i,2)];
%     x = [Ax(2) OrignX Pnt(1)];
%     y = [Ay(2) OrignY Pnt(2)];
%     
%     tmpx = [Ax(3) OrignX Pnt(1)];
...
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     if charmode ==1,
%         A = [sRGB_B(i,1) sRGB_B(i,2)];
%         B = [Target_B(i,1) Target_B(i,2)];
%         r_sq = pdist2(A,B,'euclidean');
%         if r_sq == 0.0,
%             r_sq = 0.0025;
%         else
...
% Ay = ADxy(2,:);
% 
% for i=1:1:max(size(sRGB_B)),
% %for i=1:1,
%     
%     Pnt = [sRGB_B1(i,1) sRGB_B1(i,2)];
%     x = [Ax(3) OrignX Pnt(1)];
%     y = [Ay(3) OrignY Pnt(2)];
%     
%     tmpx = [Ax(3) OrignX Pnt(1)];
